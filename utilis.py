...
def user_info(user_name):
    ...
    info = {
        ...
        "totalAmountWords": len(list(db["wordsObj"].find({"level":{"$gte":1,"$lte":user_obj["level"]}}))),
        "shouldLearn": len(list(user_collection.find({"alreadyKnow":False}))),
        "neverLearned": len(list(user_collection.find({"repetitions":-1}))),
        "startedPractice": len(list(user_collection.find({"repetitions": {"$ne":-1}}))),
        "learnedPerfectly": len(list(user_collection.find({"repetitions": {"$gte": 5}}))),
...
def encrypt_data(plaintext, salt = bytes(Random.get_random_bytes(16).hex(), 'utf-8')):
